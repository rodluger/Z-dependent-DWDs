...
\begin{figure}
	\includegraphics[width=\columnwidth]{figures/SFH_vs_fb.png}
    \caption{The metallicity-dependent binary fraction for close solar-type binaries with P $< 10^4$ days (black) plotted against the logarithm of Z/Z$_\odot$, with scatter points denoting the location of the metallicity grid used in our \cosmic\ simulations. The secondary axis shows the amount of mass in stars formed within each metallicity bin from galaxy \textbf{m12i} of the ``Latte" suite in FIRE as a red histogram. The amount of stellar mass formed with super-solar metallicities dominates the distribution. Note that the primary axis shows $f_{\rm{b}}(Z)$ in linear scale, and the secondary axis shows amount of mass formed in log-scale. The opposing trends of these two distributions compete throughout this study.}
    \label{fig:SFH_vs_fb}
\end{figure}

The metallicity-dependent binary fraction is shown in black in Figure \ref{fig:SFH_vs_fb} along with the mass in star particles from galaxy \textbf{m12i}, shown in red, as a function of our metallicity grid. The binary fraction, $f_{\rm{b}}$, drops drastically across metallicity while the mass formed in FIRE increases significantly. These two opposing trends compete within this study along with the impact of metallicity on single star evolution to form the final numerical distribution of systems in our DWD populations. 

Since our \cosmic\ simulations assume a binary fraction of $f_{\rm{b}} = 1$, we scale the amount of mass sampled at ZAMS required to generate the population of DWDs to the proper binary fraction for each model. We do this by sampling single stars and primary masses of binary stars from the \cite{Kroupa2001} IMF and sampling secondary masses of the binary stars from a uniform mass distribution, where the number of binaries is determined by the binary fraction model (F50 or FZ). From this sample, we obtain the ratio of mass in single stars to the mass in binary stars as a function of the binary fraction, $R(f_{\rm{b}})$. For model F50, the ratio is a constant $R(f_{\rm{b}}) =$ 0.64. For model FZ, the ratio increases from 0.68 at low metallicities to 3.17 at solar metallicity indicating fewer mass in binaries relative to single stars. This ratio can then be used to scale the total amount of ZAMS mass sampled in binary stars required to produce our simulated population from \cosmic\ to $M_{\rm{ZAMS,sim}}$, the total amount of ZAMS mass in single stars and binary systems required to produce the simulated population for a given binary fraction $f_{\rm{b}}$.
...
\end{equation}
where where $a_i$ is the DWD separation at formation, and 
\begin{equation}
...
\begin{equation}
    a_{\text{RLO,}\ell} = R_{\ell} \cdot \frac{0.6 q_{\ell}^{2/3} + \ln{(1+q_{\ell}^{1/3})}}{0.49 q_{\ell}^{2/3}}
\end{equation}
...
\begin{equation}
    R = \max\Bigg(R_\text{NS}, 0.0115\sqrt{\left(\frac{M_\text{Ch}}{M}\right)^{2/3}-\left(\frac{M}{M_\text{Ch}}\right)^{2/3}} \Bigg) 
\end{equation}
...
\begin{equation}
    \Dot{f}_n = \frac{48n}{5\pi} \frac{(G\mathcal{M}_c)^{5/3}}{c^5} \cdot (2\pi f_\text{orb})^{11/3}.
\end{equation}
...
\begin{equation}
    h_{c}^2 = \frac{2}{3 \pi^{4/3}} \frac{(G \mathcal{M}_c)^{5/3}}{c^3 D_L^2} \frac{1}{f_{\rm GW}^{1/3}},
\end{equation}
...
\begin{equation}
    h_2^2 = \frac{\dot{f}_{2}}{4f_{\rm{orb}}^2} h_c^2.
\end{equation}
...
\subsection{Metallicity trends in DWD progenitor Roche-overflow separation}\label{sec:CEsep}

\begin{figure*}
	\includegraphics[width=\textwidth]{figures/lisa_nums.png}
...
All systems that end up radiating GWs in the LISA band have undergone at least one phase of CEE. For systems which experience a stable RLO mass transfer in the first interaction, the CEE phase plays a key role in shrinking systems with initially wide separations to bring them into the LISA band. 

Figure \ref{fig:CEsep} shows the average separation at the first instance of RLO, $a_{\rm{RLO}}$ of all DWD progenitors that result in systems orbiting in the LISA frequency band at present day as a function of metallicity for each DWD type. The solid lines denote the average value, and the $1\sigma$ variance is shown in the surrounding shading. 

The average interaction separation increases on average across metallicity for systems that reside in the LISA band at present. Higher-metallicity binaries will interact earlier in the binary's lifetime than a lower-metallicity binary of equal separation due to their relatively larger maximum radii. Since higher metallicity binaries are also more likely to merge during RLO interactions because of their relatively more massive donor envelopes, the DWDs which survive and eventually orbit in the LISA band originate from systems with higher interaction separations which allow their orbits to shrink significantly during CEE phases without merging. This regulation plays a key role in smearing out any observable effects of a metallicity-dependent binary fraction in the population of DWDs observable by LISA. 
...
\begin{figure}
	\includegraphics[width=1\columnwidth]{figures/PSD.png}
...
Similar to previous studies, we find that LISA will be able to resolve several thousand DWDs. Figure \ref{fig:LISA_SNR} shows the amplitude spectral density vs GW frequency of the resolved systems with SNR $> 7$ for each each DWD type where the top and bottom rows show results for models FZ and F50 respectively. For comparison, we also show the LISA sensitivity curve, including the confusion foreground from each population, in black and the the entire population for each model in grey. Apart from each DWD type having a higher abundance of resolved systems, the population-wide characteristics remain unchanged between the two binary fraction models. The populations containing at least one He WD occupy the lower ASD, higher GW frequency region of parameter space compared to total population, with CO + He DWDs having larger ASDs than the He + He DWD population. Conversely, DWD types without a He WD component tend to occupy the higher ASD, lower GW frequency region of parameter space. This difference is largely due to the formation scenario of DWDs containing a He WD, which form from the ejection of a common envelope created by the He WD progenitor. These lower mass progenitors overflow their Roche lobes at closer separations relative to the higher mass progenitors (e.g. Figure~\ref{fig:CEsep}) and thus also produce closer DWDs. While the distance to any one DWD strongly influences its ASD, DWD populations without a He WD component have, on average, higher ASDs due to their more massive component WDs.

We note that while the height of the confusion foreground and the number of DWDs radiating GWs with $f_{\rm{GW}} > 10^{-4}\,\rm{Hz}$ is reduced by a factor of 2 for model FZ relative to model F50, the number of resolved sources is not reduced to an equal degree. This is because in the absence of less competing GW signals from DWDs in the LISA frequency band, more DWDs can be individually resolved. Between models FZ and F50, the number of resolvable systems with SNR $>7$ over all DWD types decreases by only $\sim14\%$. Interestingly, the number of resolvable He + He sources actually \emph{increases} for model FZ relative to F50 by $\sim 1.4\%$. Because these systems tend to have low ASD values due to their lower chirp masses, they benefit from a foreground reduction more than other DWD types.

The distance and chirp mass of DWDs which exhibit observable orbital evolution due to the emission of GWs during the LISA mission can be measured. This is because the chirp mass -- distance degeneracy in the observed strain can be broken with the observed GW frequency evolution, or chirp. Assuming a chirp resolution of $1/T_{\rm{obs}}^2 \sim 8\times10^{-9}\,\rm{Hz}^2$, we selected the DWDs whose chirp masses and distances can be measured. Figure \ref{fig:Mc_vs_dist} shows the chirp mass vs the luminosity distance for each DWD type in this selected population. The contours show the $5^{\rm{th}}$, $25^{\rm{th}}$, $50^{\rm{th}}$, $75^{\rm{th}}$, and $95^{\rm{th}}$ percentiles for models FZ (light blue) and F50 (dark blue, dashed). Despite the reduction in the height of the confusion foreground when considering model FZ relative to F50, we find that LISA is unable to differentiate between the chirp mass -- distance distributions of the two models.

\section{Binary evolution assumption variations}
\label{sec:model_compare}

\begin{figure}
	\includegraphics[width=0.45\textwidth]{figures/model_comp.png}
    \caption{The top panel shows the Galactic DWD confusion fit vs GW frequency for different binary evolution assumption variations (colors) with submodel F50 shown in dashed lines and submodel FZ shown in solid lines. The bottom panel shows the ratio of the number of DWDs orbiting in the LISA frequency band for submodel FZ to submodel F50 vs to number of DWDs orbiting in the LISA band for submodel F50 only. While both the height of the confusion foreground and number of LISA DWDs changes for each variation, the FZ submodels within each variation exhibit a constant reduction by a factor of two compared with the F50 submodels.}
    \label{fig:model_comp}
\end{figure}

In order to test the robustness of the reduction of the height of the Galactic DWD GW foreground when assuming a metallicity-dependent binary fraction, we repeated our analysis using three different binary evolution assumption variations. For each variation, we consider submodels FZ and F50 as done in the fiducial case described above. In model $q3$, we varied the assumption for the critical mass ratios at which a RLO interaction remains stable or becomes unstable from our fiducial assumptions such that the critical mass ratio is increased to $3.0$ and thus allows stable mass transfer for more massive RLO donors. In variations $\alpha25$ and $\alpha5$, we modified the common envelope ejection efficiency to be either much less ($\alpha=0.25$) or more ($\alpha=5$) than in our fiducial assumption set. Larger common envelope ejection efficiencies lead to wider post-CEE separations, while smaller ejection efficiencies either lead to closer post-CEE separations or mergers where then envelope ejection fails. 
...
Interestingly, when we compare the populations of each submodel for our variations, we find that the number of close DWDs reduces by the constant factor of $2$ seen in our fiducial set of assumptions. This is illustrated in \ref{fig:model_comp}. The top panel shows the confusion foreground fits for each binary evolution variation (different colored lines) and for each binary fraction  submodel where the solid lines show FZ submodels and dotted lines show F50 submodels. The bottom panel shows the ratio of the number of DWDs orbiting in the LISA frequency band for submodels FZ and F50 vs the total number of DWDs orbiting in the LISA band for submodel F50 only. Even though the number of DWDs in the LISA band spans over two orders of magnitude, the ratio of submodel FZ to F50, as well as the spectral shape of the confusion fit, stays fixed at a constant factor of two reduction. This suggests that assuming a metallicity-dependent binary fraction reduces the size of the Galactic close DWD population by a factor of $\sim$ two and the strength of the Galactic DWD GW foreground for LISA \emph{regardless of the chosen binary evolution model}.
...
\software{astropy \citep{astropy:2013, astropy:2018}; 
          ...
          \legwork\ (legwork.readthedocs.io);
